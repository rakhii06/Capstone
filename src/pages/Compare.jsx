  ...
  if (compare.length === 0) {
    return (
      ...
      bg-white text-black dark:bg-black dark:text-white">
        ...
        <h2 className="text-3xl neon-text-cyan mb-4">
    ...
    ">
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse">
          ...
          <thead>
            <tr>
              <th className="text-left p-3 text-gray-600 dark:text-zinc-400 font-medium w-32">
                Feature
              </th>
              ...
              {compare.map((p) => (
                <th key={p.id} className="p-3 text-center align-top">
                  ...
                  ">
                    ...
                    <Link
                      to={`/product/${p.id}`}
                      className="block text-black dark:text-white font-semibold hover:neon-text-cyan truncate"
                    >
                      {p.title}
                    </Link>
          ...
          <tbody>
            {rows.map((row) => (
              <tr
                key={row.label}
                className="border-t border-gray-300 dark:border-zinc-800"
              >
                <td className="p-3 text-gray-600 dark:text-zinc-400 font-medium">
                  {row.label}
                </td>
